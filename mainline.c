 ...
 */
static int loadAssociativeArray(AssociativeArray *assocArray, char *filename, int useIntKey){
	...
	fp = fopen(filename, "r");
	if (fp == NULL) {
		fprintf(stderr, "Error: Failed to open input file '%s' : %s", filename, strerror(errno));
		return -1;
	}
	...
	while (readDataLine(fp, linebuffer, LINE_MAX, &strkey, &value) > 0){
		if (useIntKey && isdigit(strkey[0])) {
			...
			if (aaInsert(assocArray,(AAKeyType) &intkey, sizeof(int), strdup(value)) < 0) {
				fprintf(stderr, "[1] Failed to add key '%d' to assocArray\n", intkey);
				return -1;
			}
		} else {

			if (aaInsert(assocArray, (AAKeyType) strkey, strlen(strkey), strdup(value)) < 0) {
				fprintf(stderr, "[2] Failed to add key '%s' to assocArray\n", strkey);
				return -1;
			}
		}
		nEntries++;
 ...
 */
static int queryAssociativeArray(AssociativeArray *assocArray, char *filename, int useIntKey){
	char linebuffer[LINE_MAX];
	char *strkey = NULL, *value = NULL;
	int intkey;
	FILE *fp = NULL;
	...
	fp = fopen(filename, "r");
	if (fp == NULL) {
		fprintf(stderr, "Error: Failed to open query input file '%s' : %s", filename, strerror(errno));
		return -1;
	}
 ...
 */
static int deleteFromAssociativeArray(AssociativeArray *assocArray, char *filename, int useIntKey){
...
static int deleteValue(AAKeyType key, size_t keylen, void *value, void *userdata){
	...
	while ((c = getopt(argc, argv, "hpin:o:P:H:2:q:d:")) != -1) {
		if (c == 'i') {
			...
			if (sscanf(optarg, "%d", &arraySize) != 1) {
				fprintf(stderr,"Error: cannot parse assocArray size requested from '%s'\n", optarg);
				usage(programname);
			}
		} else if (c == 'H') {
			hash1 = optarg;
			...
			ofp = fopen(optarg, "w");
			if (ofp == NULL) {
				fprintf(stderr,"Error: cannot open requested output file '%s' : %s\n", optarg, strerror(errno));
				usage(programname);
			}
