...
int readDataLine(FILE *dataFP, char *line, int maxlinelen, char **key, char **value){
	...
	if (delimiterPosition == NULL) {
		 ...
		 */
		fprintf(stderr,"Error: Input line does not contain" "delimiter char '%c': '%s'\n", DELIMITER_CHAR, line);
		return -1;
	}
 ...
 * Where does it get allocated?  Where does it get deallocated?
 */
int readPlainLine(FILE *dataFP, char *line, int maxlinelen, char **value){
	/** read the file until empty */
	if (fgets(line, maxlinelen, dataFP) == NULL) {
		return 0;
	}
 ...
 */
static int dataCharacter(char c){
